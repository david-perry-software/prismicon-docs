...
- [ ] 1.1 Confirm baseline and lock scaffold-only boundaries against the accepted decisions; verify: `npm test && npm run check:variants`
...
- [ ] 2.1 Add a deterministic Wright scaffold variant module with spec framing and minimal contract-valid hooks only; verify: focused Wright variant tests pass under `npm test`
- [ ] 2.2 Register the Wright scaffold in built-in variant wiring without changing default variant behavior; verify: focused registry/resolve tests pass under `npm test`
- [ ] 2.3 Validate deterministic behavior and error contract parity for the new id in static and mounted entry points; verify: focused dispatch/variant tests pass under `npm test`
