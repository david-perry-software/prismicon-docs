...
- Concurrent-delivery check: on 2026-09-21 the product repo has two open draft PRs — this delivery (`#22`) and `#21` `feature/wright-variant-motion-events` (an empty planning draft; `gh pr diff 21 --name-only` shows no files yet). The companion repo has `#7` (this delivery) and no other overlapping draft. Both features will edit `src/variants/wright.js` and `test/wright.test.js`, but in disjoint regions (motion = `pose`/`animate`/`flash`; legibility = `prepare`/`buildWright`) and disjoint `test()` blocks. The breakdown lists this member `Recommended after: wright-variant-motion-events`. Recheck before every push and integrate `origin/main` as required.
...
- Concurrent delivery overlap: `#21` `feature/wright-variant-motion-events` is in flight and touches the same two files (`src/variants/wright.js`, `test/wright.test.js`). Mitigation: the edits are disjoint (motion vs prepare/geometry) but integration must be ordered — integrate `origin/main` before every push, and if motion-events ships mid-build, merge it in and rerun `node --test test/wright.test.js` plus `npm run verify`; sequence this feature after `wright-variant-motion-events` where the schedule allows, per the breakdown's `Recommended after`.
