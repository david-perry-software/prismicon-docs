...
- [x] 2.4 Enforce stroke-aware 100-by-100 viewBox bounds at sizes 24, 64, and 72 without adding aesthetic detail reduction; verify: focused `test/wright.test.js` size-matrix assertions pass and rendered markup contains neither `NaN` nor `Infinity`
...
- [x] 3.3 Run the complete no-lint replacement gate after integrating `origin/main`; verify: `node -e "const p=require('./package.json'); if ('lint' in p.scripts) process.exit(1)" && npm run verify` exits 0, including contract, exports, types, pack, and unchanged existing goldens