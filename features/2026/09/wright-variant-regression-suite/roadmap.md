...
- [ ] 2.1 Extend `test/helpers/golden.js` with a Wright-only seed extension so `captureGolden({ variant: 'wright' })` additionally captures the representative static seeds and mounted scenarios (one per geometry family, one per palette family, one concrete hybrid with `deriveWright(seed).secondaryFamily !== null`), leaving the shared `STATIC_SEEDS`/`MOUNTED_SCENARIOS` and every other family untouched — verify: `node --test test/golden-v1.test.js test/golden-ncube.test.js test/golden-orbit.test.js test/wright.test.js`
